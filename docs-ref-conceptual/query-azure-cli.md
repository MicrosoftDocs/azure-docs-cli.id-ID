...
description: Azure CLI menggunakan argumen --query untuk mengeksekusi kueri JMESPath pada hasil perintah. Pelajari cara menggunakan fitur JMESPath dalam artikel ini.
...
Azure CLI menggunakan `--query` argumen untuk mengeksekusi kueri [JMESPath](http://jmespath.org) pada hasil perintah. JMESPath adalah bahasa kueri untuk JSON, memberi Anda kemampuan untuk memilih dan memodifikasi data dari output CLI. Kueri dijalankan pada output JSON sebelum pemformatan tampilan apa pun.

Argumen ini `--query` didukung oleh semua perintah dalam Azure CLI. Artikel ini membahas cara menggunakan fitur JMESPath dengan serangkaian contoh kecil dan sederhana.

> [!NOTE]
>
> Saat menggunakan Azure CLI di PowerShell pada Windows, beberapa pelarian tambahan mungkin diperlukan untuk argumen kueri. Silakan lihat [Mengutip masalah dengan PowerShell](https://github.com/Azure/azure-cli/blob/dev/doc/quoting-issues-with-powershell.md) untuk detail lebih lanjut.

## <a name="dictionary-and-list-cli-results"></a>Kamus dan daftar hasil CLI

Bahkan ketika menggunakan format output selain JSON, hasil perintah CLI pertama kali diperlakukan sebagai JSON untuk kueri. Hasil CLI adalah array JSON atau kamus. Array adalah urutan objek yang dapat diindeks, dan kamus adalah objek yang tidak teratur yang diakses dengan kunci. Perintah yang _dapat_ mengembalikan lebih dari satu objek mengembalikan array, dan perintah yang _selalu_ _mengembalikan hanya_ satu objek mengembalikan kamus.

## <a name="get-properties-in-a-dictionary"></a>Mendapatkan properti dalam kamus

Bekerja dengan hasil kamus, Anda dapat mengakses properti dari tingkat atas hanya dengan kunci. Karakter `.` __(subexpression)__ digunakan untuk mengakses sifat kamus bersarang. Sebelum memperkenalkan kueri, lihat output perintah yang tidak `az vm show` dimodifikasi:
...
Perintah berikut membuat kunci publik SSH diizinkan untuk terhubung ke VM dengan menambahkan kueri:
...
## <a name="get-a-single-value"></a>Dapatkan satu nilai
...
Untuk informasi selengkapnya tentang `tsv` format output, lihat [Format output - format output TSV](format-output-azure-cli.md#tsv-output-format)

## <a name="query-boolean-values"></a>Nilai Kueri Boolean

Querying nilai Boolean sedikit berbeda.  Ada dua opsi:
...
Untuk mendapatkan lebih dari satu properti, letakkan ekspresi dalam tanda kurung siku  `[ ]` __(daftar multiselect)__ sebagai daftar yang dipisahkan koma. Untuk mendapatkan nama VM, pengguna admin, dan kunci SSH sekaligus gunakan perintah:
...
Untuk mendapatkan kamus, bukan array saat meminta beberapa nilai, gunakan `{ }` operator __(multiselect hash).__
...
Array tidak memiliki sifat sendiri, tetapi dapat diindeks. Fitur ini ditampilkan dalam contoh terakhir dengan `publicKeys[0]` ekspresi, yang mendapat elemen pertama dari `publicKeys` array. Tidak ada jaminan output CLI dipesan, jadi hindari menggunakan pengindeksan kecuali Anda yakin pesanan atau tidak peduli elemen apa yang Anda dapatkan. Untuk mengakses properti elemen dalam array, Anda melakukan salah satu dari dua operasi: _meratakan_ dan _memfilter._ Bagian ini mencakup cara meratakan array.

Meratakan array dilakukan dengan `[]` operator JMESPath. Semua ekspresi setelah `[]` operator diterapkan pada setiap elemen dalam array saat ini.
Jika `[]` muncul di awal kueri, kueri meratakan hasil perintah CLI. Hasilnya `az vm list` dapat diperiksa dengan fitur ini.
...
> [!NOTE]
>
> Tombol tertentu difilter dan tidak dicetak dalam tampilan tabel. Kunci-kunci ini `id` `type` adalah, dan `etag` . Untuk melihat nilai-nilai ini, Anda dapat mengubah nama kunci dalam hash multiselect.
>
> ```azurecli-interactive
> az vm show -g QueryDemo -n TestVM --query "{objectID:id}" -o table
> ```

Array apa pun dapat diratakan, bukan hanya hasil tingkat atas yang dikembalikan oleh perintah. Pada bagian terakhir, ekspresi `osProfile.linuxConfiguration.ssh.publicKeys[0].keyData` digunakan untuk mendapatkan kunci publik SSH untuk masuk. Untuk mendapatkan _setiap_ kunci publik SSH, ekspresinya malah bisa ditulis sebagai `osProfile.linuxConfiguration.ssh.publicKeys[].keyData` .
...
Operasi lain yang digunakan untuk mendapatkan data dari array adalah _penyaringan._ Penyaringan dilakukan dengan `[?...]` operator JMESPath.
Operator ini mengambil predikat sebagai isinya. Predikat adalah pernyataan apa pun yang dapat dievaluasi untuk salah `true` satu atau `false` . Ekspresi di mana predikat mengevaluasi untuk `true` dimasukkan dalam output.

JMESPath menawarkan perbandingan standar dan operator logis. Ini `<` termasuk, `<=` , , , , dan `>` `>=` `==` `!=` .
JMESPath juga mendukung logika dan ( `&&` ), atau ( ), dan bukan `||` `!` (). Ekspresi dapat dikelompokkan dalam kurung, memungkinkan ekspresi predikat yang lebih kompleks. Untuk detail lengkap tentang predikat dan operasi logis, lihat [spesifikasi JMESPath.](http://jmespath.org/specification.html)

Di bagian terakhir, kami meratakan array untuk mendapatkan daftar lengkap semua VM dalam grup sumber daya. Dengan menggunakan filter, output ini hanya dapat dibatasi untuk VM Linux:
...
JMESPath juga memiliki fungsi bawaan yang dapat membantu penyaringan. Salah satu fungsi tersebut `? contains(string, substring)` adalah, yang memeriksa untuk melihat apakah string berisi substring. Ekspresi dievaluasi sebelum memanggil fungsi, sehingga argumen pertama bisa menjadi ekspresi JMESPath penuh. Contoh berikutnya menemukan semua VM menggunakan penyimpanan SSD untuk disk OS mereka:
...
Pertanyaan ini sedikit panjang. `storageProfile.osDisk.managedDisk.storageAccountType`Kunci disebutkan dua kali, dan rekeyed dalam output. Salah satu cara untuk mempersingkatnya adalah dengan menerapkan filter setelah meratakan dan memilih data.
...
Fungsi JMESPath juga memiliki tujuan lain, yaitu untuk beroperasi pada hasil kueri. Fungsi apa pun yang mengembalikan nilai non-boolean mengubah hasil ekspresi.
Misalnya, Anda dapat mengurutkan data berdasarkan nilai properti dengan `sort_by(array, &sort_expression)` . JMESPath menggunakan operator `&` khusus, untuk ekspresi yang harus dievaluasi kemudian sebagai bagian dari fungsi. Contoh berikutnya menunjukkan cara mengurutkan daftar VM menurut ukuran disk OS:
